...
const getFileIcon = (filename: string) => {
  ...
  switch (ext) {
    ...
    default:
      ...
      return <File className="w-4 h-4 m3-text-secondary" />;
  }
};
    ...
    return (
      <>
        {parts.map((part, i) => {
          ...
          return isMatch ? (
            <span key={i} className="bg-yellow-500/20 m3-text-primary font-bold px-0.5 rounded">
              {part}
            </span>
          ) : (
    ...
    <div className="m3-card h-full">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-[var(--md-sys-color-primary)]">
          ...
          <h3 className="font-bold text-lg m3-text-primary">File Manager</h3>
        </div>
      </div>
      
      <div className="relative flex items-center gap-2">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 m3-text-secondary" />
          <input
            type="text"
            ...
            className="m3-input m3-input-with-icon"
          />
          {searchTerm && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold bg-[var(--md-sys-color-surface-container-high)] m3-text-secondary px-2 py-0.5 rounded-full border border-[var(--md-sys-color-outline-variant)] pointer-events-none">
        ...
        <button
          ...
          className={`shrink-0 w-12 h-12 !p-0 ${isCaseSensitive ? 'm3-button-filled' : 'm3-button-tonal'}`}
        >
          Aa
        </button>
      </div>
      ...
      {recentSearches.length > 0 && !searchTerm && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="text-xs font-bold m3-text-secondary uppercase tracking-wider flex items-center gap-1">
              <Clock className="w-3 h-3" /> Recent Searches
            </div>
            <button 
              onClick={clearHistory}
              className="m3-button-text text-xs min-h-[32px] px-4 uppercase font-bold"
            >
              Clear
            </button>
          </div>
          <div className="flex flex-wrap gap-2">
            {recentSearches.map((term, idx) => (
              <button
                ...
                className="m3-button-tonal text-sm font-normal min-h-[32px] px-3 py-1"
              >
                <Search className="w-4 h-4 m3-text-secondary" />
                {term}
              </button>
            ))}
          </div>
      ...
      <div className="flex items-center px-4 py-2 bg-[var(--md-sys-color-surface-container-high)] rounded-xl border border-[var(--md-sys-color-outline-variant)]">
        <button 
          className="m3-button-text flex-1 justify-start !text-xs !font-bold m3-text-secondary uppercase tracking-wider !px-2 !min-h-[32px]"
        ...
        <button 
          className="m3-button-text w-24 justify-end !text-xs !font-bold m3-text-secondary uppercase tracking-wider !px-2 !min-h-[32px]"
        ...
        <AnimatePresence>
          {filteredFiles.map((file, idx) => (
            ...
            >
              <div className="flex-1 flex items-center gap-4">
                {getFileIcon(file)}
                <span className="text-sm m3-text-primary font-mono">
                  {highlightMatch(file)}
                </span>
              </div>
              <span className="text-xs m3-text-secondary uppercase font-bold w-24 text-right">
          ...
          {filteredFiles.length === 0 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center p-4 text-sm m3-text-secondary"
            >
    ...
    <div className="m3-card h-full">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-lg m3-text-primary">System Resources</h3>
        </div>
      </div>
          ...
          >
            <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm m3-text-primary">{alertState.message}</p>
              </div>
              <button 
                onClick={() => setAlertState({ active: false, message: '' })}
                className="m3-button-tonal !w-11 !h-11 !p-0 !text-red-500 !bg-red-500/10 !border-red-500/30 hover:!bg-red-500/20 shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-[var(--md-sys-color-surface-container-high)] p-4 rounded-2xl border border-[var(--md-sys-color-outline-variant)] flex flex-col">
          <div className="flex items-center gap-2 m3-text-secondary text-xs font-bold uppercase tracking-wider mb-2">
  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar p-6 bg-[var(--md-sys-color-background)]">
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between mb-8">
        <div>
          <h2 className="text-3xl font-bold tracking-tight m3-text-primary">Dashboard Panel</h2>
          <p className="text-sm m3-text-secondary mt-1">System overview and control center</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            onClick={toggleTheme}
            className="m3-button-tonal w-12 h-12 !p-0 rounded-full"
          >
              ...
              >
                <div className="m3-card">
                  <div className="flex items-center gap-2 text-[var(--md-sys-color-secondary)]">
                    ...
                    <h3 className="font-bold text-lg m3-text-primary">Server Panel</h3>
                  </div>
                  <p className="text-sm m3-text-secondary">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-mono font-bold m3-text-primary">STATUS: ONLINE</span>
                  </div>
                </div>
              </motion.div>
            )}
