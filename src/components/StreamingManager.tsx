          ...
          <div className="flex items-center gap-4">
             <button 
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl border font-black text-[10px] uppercase tracking-widest transition-all ${
                autoStreaming 
                ? "bg-blue-500/10 border-blue-500/50 text-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)]" 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div className="bg-[#121214] border border-[#29292E] p-8 rounded-[32px] space-y-4">
                  <div className="flex items-center justify-between">
                     ...
                     <span className="text-[10px] font-black text-[#4D4D57] uppercase tracking-widest">Total Managed</span>
                  </div>
                  <div className="flex items-baseline gap-2">
                     <span className="text-4xl font-black text-white tracking-tighter">{assets.length}</span>
                     <span className="text-xs font-bold text-[#4D4D57]">Assets / {assets.filter(a => a.status === 'LOADED').length} Active</span>
                  </div>
               </div>
            </div>

            <div className="bg-[#0A0A0B] border border-[#29292E] rounded-[40px] overflow-hidden">
               <div className="p-8 border-b border-[#29292E] flex items-center justify-between">
                  <h3 className="text-xs font-black text-white uppercase tracking-widest">Asset Registry & Proximity</h3>
                  <div className="flex items-center gap-4">
                    <button 
                      onClick={() => {
                      ...
                      className="flex items-center gap-2 text-[10px] font-black text-blue-500 hover:text-blue-400 transition-colors"
                    >
                    ...
                    >
                      <div className="flex items-center gap-6">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${
                          asset.status === 'LOADED' ? "bg-blue-500/20 text-blue-500" : "bg-[#121214] text-[#4D4D57]"
                        }`}>
                          <Box className="w-7 h-7" />
                        </div>
                        <div className="space-y-1">
                          <h4 className="text-sm font-bold text-white uppercase tracking-tight">{asset.id}</h4>
                          <p className="text-[10px] text-[#4D4D57] font-mono leading-none">{asset.path}</p>
                          <div className="flex items-center gap-3 mt-2">
                             <span className="text-[9px] font-black text-[#8D8D99] bg-[#121214] px-2 py-0.5 rounded cursor-help" title={`Radius: ${asset.loadRadius}`}>
                                RAD: {asset.loadRadius}
                             </span>
                             <span className="text-[9px] font-black text-[#8D8D99] bg-[#121214] px-2 py-0.5 rounded">
                      ...
                      <div className="flex items-center gap-8">
                         ...
                         <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                            ...
                            <button 
                              ...
                              className="p-2 hover:bg-red-500/10 text-[#4D4D57] hover:text-red-500 rounded-xl transition-colors"
                            >
          ...
          <aside className="space-y-8">
            <div className="bg-[#121214] border border-[#29292E] rounded-[32px] p-8 space-y-6">
               <div className="flex items-center gap-3 border-b border-[#29292E] pb-4">
                  ...
                  <h3 className="text-xs font-black text-white uppercase tracking-[0.2em]">Cálculo de Carga</h3>
               </div>
               <div className="space-y-6">
                  <div className="space-y-2">
                     <p className="text-[10px] text-[#4D4D57] font-black uppercase tracking-widest">Algoritmo de Proximidade</p>
                  ...
                  <div className="p-6 bg-black/40 border border-white/5 rounded-2xl flex items-center justify-between">
                     <span className="text-[10px] font-black text-[#4D4D57] uppercase">VRAM Efficiency</span>
            ...
            <div className="bg-gradient-to-br from-blue-500/10 to-transparent border border-blue-500/20 rounded-[32px] p-8 space-y-6">
               <h3 className="text-xs font-black text-white uppercase tracking-[0.2em]">Live Stream Debug</h3>
               <div className="space-y-4">
                  {assets.filter(a => a.distance! < a.loadRadius + 2000).slice(0, 3).map(a => (
                    <div key={a.id} className="flex items-center justify-between text-xs">
                       <span className="text-[#8D8D99] truncate mr-2">{a.id}</span>
                       <span className={`font-mono text-[10px] ${a.status === 'LOADED' ? "text-blue-400" : "text-amber-500"}`}>
                          {(a.distance! / a.loadRadius * 100).toFixed(0)}% PROX
                       </span>
                    </div>
                  ))}
               </div>
               <button className="w-full py-4 bg-blue-500 hover:bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-500/20 transition-all">
                  FORCE_RECALC_NODES
               </button>
            </div>
          </aside>
